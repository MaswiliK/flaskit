...
def create(
    ...
    if template not in ("mvp", "saas"):
        console.print(
            f"[yellow]⚠️ Unknown template '{template}', defaulting to 'mvp'[/]"
        )
        template = "mvp"
    ...
    console.print(
        Panel(
            f"🌱 Creating project [bold cyan]{project_name}[/] using [bold yellow]{template.upper()}[/] template",
            expand=False,
        )
    )
    ...
    console.print(
        Panel(
            f"✅ Project [bold cyan]{project_name}[/] ready! Start coding!",
            style="green",
        )
    )
...
def demo():
    ...
    if root.exists():
        console.print(
            f"[yellow]⚠️ '{project}' already exists. Remove it first or run from another directory.[/]"
        )
        raise typer.Exit(1)

    console.print(
        Panel(
            "⚡ FlaskIt Demo — watch a project come to life in seconds",
            style="bold cyan",
            expand=False,
        )
    )
...
def up(
    ...
    if (root / "run.py").exists():
        ...
        console.print(
            "[red]❌ Error: No Flask entry point found (main.py or run.py)[/]"
        )
    ...
    if not venv_python.exists():
        console.print(
            f"[red]❌ Error: Virtual environment Python not found at {venv_python}[/]"
        )
        console.print(
    ...
    if not skip_deps:
        ...
        if requirements_file.exists():
            ...
            if pip_upgrade.returncode != 0:
                console.print(
                    f"  [yellow]⚠️ Could not upgrade pip: {pip_upgrade.stderr}[/]"
                )
            ...
            if result.returncode != 0:
                console.print(f"[red]❌ Failed to install dependencies:[/]")
            ...
            if result.returncode != 0:
                console.print(f"[red]❌ Failed to install Flask:[/]")
                console.print(f"[red]{result.stderr}[/]")
                raise typer.Exit(1)
            console.print("  [green]✓[/] Flask installed")
    else:
        console.print(
    ...
    if env_file.exists():
        ...
        try:
            ...
            console.print("  [green]✓[/] .env created from .env.example")
        except Exception as e:
            console.print(f"[red]❌ Failed to create .env: {e}[/]")
            raise typer.Exit(1)
    else:
        ...
        try:
            ...
            console.print("  [green]✓[/] Created basic .env file")
        except Exception as e:
            console.print(f"[red]❌ Failed to create .env: {e}[/]")
            raise typer.Exit(1)
...
"""
            ...
            if result.returncode == 0:
                console.print(
                    f"  [green]✓[/] {result.stdout.strip() or 'Database initialized'}"
                )
            else:
                ...
                if (
                    ...
                    console.print(
                        f"  [yellow]⚠️ Database initialization warning: {result.stderr.strip()}[/]"
                    )
        else:
    ...
    if not skip_migrate:
        ...
        if migrations_dir.exists():
            ...
            if result.returncode == 0:
                ...
                console.print(
                    f"  [yellow]⚠️ Migration warning: {result.stderr.strip() or 'No migrations to apply'}[/]"
                )
        else:
    ...
    if not no_server:
        ...
        console.print(
            Panel(
                "[green]✓ FlaskIt project is running at http://127.0.0.1:5000[/]",
                expand=False,
            )
        )
